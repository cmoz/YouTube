...
static esp_err_t lvgl_init()  // Initialize LVGL
{
    ...
    lvgl_port_display_cfg_t disp_cfg = { 0 };  // LVGL display configuration
    disp_cfg.io_handle = mipi_dbi_io;
    disp_cfg.panel_handle = panel_handle;
    disp_cfg.control_handle = panel_handle;
    disp_cfg.buffer_size = (H_size * V_size * ((BITS_PER_PIXEL + 7) / 8));
    disp_cfg.double_buffer = true;
    disp_cfg.hres = H_size;
    disp_cfg.vres = V_size;
    ...
    lvgl_port_display_dsi_cfg_t lvgl_dpi_cfg = { 0 };  // LVGL DSI configuration
#if CONFIG_DISPLAY_LVGL_AVOID_TEAR
    lvgl_dpi_cfg.flags.avoid_tearing = true;
#else
    lvgl_dpi_cfg.flags.avoid_tearing = false;
#endif
