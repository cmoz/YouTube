...
#define BUSY_PIN 2  // Busy ********* FUTURE VERSIONS SWAP THIS TO PIN 3!!!!!! ******************
#define MOSI_PIN 4  // 6 SPI MOSI
#define SCK_PIN 10  // S2 10   scl   //S2 14    SPI Clock
#define WAKE_BUTTON_PIN 0 // ******* FUTURE VERSION SWAP TO HARDWARE PIN 0 FOR WAKE BUTTON, CURRENTLY USING GPIO2 FOR BUSY TO AVOID FLASHING ISSUES ******************
...
const char *webLogoUrl = "https://cmoz.fashion/img/logo/Logo_LG_BG_crop2.png";
...
void handleRoot() {
  ...
  server.sendContent(F(".hero-logo { display: block; width: min(220px, 70vw); height: auto; margin: 0 auto 18px auto; }"));
  server.sendContent(F(".hero-logo-fallback { display: none; text-align: center; font-size: 13px; letter-spacing: 0.08em; margin: 0 auto 18px auto; }"));
  ...
  server.sendContent("<img class='hero-logo' src='" + String(webLogoUrl) + "' alt='CMOZ logo' onerror=\"this.style.display='none';var fallback=document.getElementById('hero-logo-fallback');if(fallback){fallback.style.display='block';}\">");
  server.sendContent(F("<div id='hero-logo-fallback' class='hero-logo-fallback'>CMOZ</div>"));
